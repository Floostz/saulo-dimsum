...
type IconName =
    | "grid" | "shopping-cart" | "package" | "credit-card"
    | "trending-up" | "archive" | "bar-chart-2" | "users"
    | "log-out" | "menu" | "x" | "chevron-right";
...
const Icon = (props: IconProps) => {
    ...
    return (
        <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" innerHTML={icons[props.name] ?? ""}/>
    );
};
            ...
            <aside class={[styles.sidebar, props.mobileOpen && styles.mobileOpen].filter(Boolean).join(" ")}>
                ...
                <nav class={styles.nav}>
                    <div class={styles.navSection}>
                        ...
                        <For each={navItems}>
                            {(item) => (
                                <Show when={!item.adminOnly || isAdmin()}>
