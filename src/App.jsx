...
function ContentSkeleton() {
    return (<div style={{ padding: "32px", display: "flex", "flex-direction": "column", gap: "16px" }}>
      {[1, 2, 3].map(() => (<div class="skeleton" style={{ height: "72px", "border-radius": "12px" }}/>))}
    </div>);
}
// ── Auth guard ───────────────────────────────────────────────────────────────
function Protected(props) {
    ...
    return (<Show when={!loading()} fallback={<Spinner />}>
      <Show when={user()} fallback={<RedirectTo href="/login"/>}>
        <Show when={!props.adminOnly || user()?.role === "admin"} fallback={<AccessDenied />}>
          <AppLayout>
...
function AccessDenied() {
    return (<AppLayout>
      <div style={{
        ...
        <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="var(--color-danger)" stroke-width="1.5">
...
function LoginRoute() {
    const { user, loading } = useAuth();
    return (<Show when={!loading()} fallback={<Spinner />}>
      <Show when={!user()} fallback={<RedirectTo href="/dashboard"/>}>
