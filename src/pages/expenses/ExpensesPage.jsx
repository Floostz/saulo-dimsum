...
export default function ExpensesPage() {
    ...
    const [expenses, { refetch }] = createResource(() => ({ start: startDate(), end: endDate() }), async ({ start, end }) => {
        try {
            return await getExpenses(start || undefined, end || undefined);
        }
        catch {
            return [];
        }
    ...
    const list = createMemo(() => (expenses() ?? []).filter((e) => {
    ...
    const openEdit = (e) => {
        setEditingId(e.id);
        const d = e.date instanceof Date ? e.date : e.date.toDate?.() ?? new Date(e.date);
    ...
    return (<div class={[styles.page, "page-enter"].join(" ")}>
      ...
      <div class={styles.filterRow}>
        <Input placeholder="Cari pengeluaran atau supplier..." value={search()} onInput={(e) => setSearch(e.currentTarget.value)} class={styles.searchBox} leftIcon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>}/>
        <Select options={[{ value: "", label: "Semua Kategori" }, ...CATEGORIES.map((c) => ({ value: c, label: c }))]} value={filterCat()} onChange={(e) => setFilterCat(e.currentTarget.value)} class={styles.filterSelect}/>
        <Input type="date" label="" placeholder="Dari tanggal" value={startDate()} onInput={(e) => setStartDate(e.currentTarget.value)} class={styles.dateInput}/>
        <Input type="date" label="" placeholder="Sampai" value={endDate()} onInput={(e) => setEndDate(e.currentTarget.value)} class={styles.dateInput}/>
      ...
      <Modal open={showModal()} onClose={() => setShowModal(false)} title={editingId() ? "Edit Pengeluaran" : "Tambah Pengeluaran"} size="md" footer={<><Button variant="secondary" onClick={() => setShowModal(false)}>Batal</Button><Button loading={saving()} onClick={handleSave}>{editingId() ? "Simpan" : "Tambah"}</Button></>}>
        <div class={styles.formGrid}>
          <Input label="Deskripsi" placeholder="cth. Pembelian tepung terigu" value={form().description} onInput={(e) => setForm((f) => ({ ...f, description: e.currentTarget.value }))} class={styles.colSpan2}/>
          <Select label="Kategori" options={CATEGORIES.map((c) => ({ value: c, label: c }))} placeholder="Pilih kategori" value={form().category} onChange={(e) => setForm((f) => ({ ...f, category: e.currentTarget.value }))}/>
          <Input label="Jumlah (Rp)" type="number" placeholder="0" value={form().amount || ""} onInput={(e) => setForm((f) => ({ ...f, amount: parseFloat(e.currentTarget.value) || 0 }))}/>
          <Input label="Supplier (opsional)" placeholder="Nama supplier" value={form().supplier} onInput={(e) => setForm((f) => ({ ...f, supplier: e.currentTarget.value }))}/>
          <Input label="Tanggal" type="date" value={form().date} onInput={(e) => setForm((f) => ({ ...f, date: e.currentTarget.value }))}/>
        </div>
      </Modal>

      <Modal open={!!showDelete()} onClose={() => setShowDelete(null)} title="Hapus Pengeluaran" size="sm" footer={<><Button variant="secondary" onClick={() => setShowDelete(null)}>Batal</Button><Button variant="danger" loading={deleting()} onClick={() => handleDelete(showDelete())}>Hapus</Button></>}>
