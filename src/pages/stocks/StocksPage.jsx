...
export default function StocksPage() {
    const [stocks, { refetch }] = createResource(async () => {
        try {
    ...
    const [showDelete, setShowDelete] = createSignal(null);
    const [deleting, setDeleting] = createSignal(false);
    const [showAdjust, setShowAdjust] = createSignal(null);
    const [adjustQty, setAdjustQty] = createSignal(0);
    const [adjustType, setAdjustType] = createSignal("add");
    const [adjustNote, setAdjustNote] = createSignal("");
    const [adjustSaving, setAdjustSaving] = createSignal(false);
    const list = createMemo(() => (stocks() ?? []).filter((s) => s.name.toLowerCase().includes(search().toLowerCase())));
    const lowCount = createMemo(() => (stocks() ?? []).filter((s) => s.currentStock <= s.minStock).length);
    const status = (s) => s.currentStock === 0 ? "empty" : s.currentStock <= s.minStock ? "low" : "ok";
    ...
    return (<div class={[styles.page, "page-enter"].join(" ")}>
      ...
      <Input placeholder="Cari bahan..." value={search()} onInput={(e) => setSearch(e.currentTarget.value)} class={styles.searchBox} leftIcon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>}/>
      ...
      <Show when={!stocks.loading && list().length > 0}>
        <div class={styles.stockGrid}>
          <For each={list()}>
            ...
            return (<div class={[styles.stockCard, styles[`status-${s}`]].join(" ")}>
      ...
      {/* Add/Edit Modal */}
      <Modal open={showModal()} onClose={() => setShowModal(false)} title={editingId() ? "Edit Item Stok" : "Tambah Item Stok"} size="sm" footer={<><Button variant="secondary" onClick={() => setShowModal(false)}>Batal</Button><Button loading={saving()} onClick={handleSave}>Simpan</Button></>}>
        <div class={styles.formGrid}>
          <Input label="Nama Bahan" placeholder="cth. Tepung Terigu" value={form().name} onInput={(e) => setForm((f) => ({ ...f, name: e.currentTarget.value }))} class={styles.colSpan2}/>
          <Input label="Satuan" placeholder="kg, liter, pcs..." value={form().unit} onInput={(e) => setForm((f) => ({ ...f, unit: e.currentTarget.value }))}/>
          <Input label="Stok Saat Ini" type="number" placeholder="0" value={form().currentStock || ""} onInput={(e) => setForm((f) => ({ ...f, currentStock: parseFloat(e.currentTarget.value) || 0 }))}/>
          <Input label="Stok Minimum (Alert)" type="number" placeholder="0" value={form().minStock || ""} onInput={(e) => setForm((f) => ({ ...f, minStock: parseFloat(e.currentTarget.value) || 0 }))}/>
          <Input label="Harga per Satuan (Rp)" type="number" placeholder="0" value={form().costPerUnit || ""} onInput={(e) => setForm((f) => ({ ...f, costPerUnit: parseFloat(e.currentTarget.value) || 0 }))}/>
        </div>
      </Modal>

      {/* Adjust Modal */}
      <Modal open={!!showAdjust()} onClose={() => setShowAdjust(null)} title={`Sesuaikan Stok — ${showAdjust()?.name}`} size="sm" footer={<><Button variant="secondary" onClick={() => setShowAdjust(null)}>Batal</Button><Button loading={adjustSaving()} onClick={handleAdjust}>Simpan</Button></>}>
        <div style={{ display: "flex", "flex-direction": "column", gap: "var(--space-4)" }}>
          ...
          <Input label={`Jumlah (${showAdjust()?.unit})`} type="number" placeholder="0" value={adjustQty() || ""} onInput={(e) => setAdjustQty(parseFloat(e.currentTarget.value) || 0)}/>
          <Input label="Keterangan (opsional)" placeholder="Alasan penyesuaian..." value={adjustNote()} onInput={(e) => setAdjustNote(e.currentTarget.value)}/>
          <Show when={adjustQty() > 0}>
            <div class={styles.adjustPreview}>
              Stok setelah: <strong>
                {adjustType() === "add"
            ? (showAdjust()?.currentStock ?? 0) + adjustQty()
            : Math.max(0, (showAdjust()?.currentStock ?? 0) - adjustQty())} {showAdjust()?.unit}
              </strong>
            </div>
          </Show>
        </div>
      ...
      <Modal open={!!showDelete()} onClose={() => setShowDelete(null)} title="Hapus Item Stok" size="sm" footer={<><Button variant="secondary" onClick={() => setShowDelete(null)}>Batal</Button><Button variant="danger" loading={deleting()} onClick={() => handleDelete(showDelete())}>Hapus</Button></>}>
