...
export default function ReportsPage() {
  const [reportType, setReportType] = createSignal<ReportType>("monthly");
  ...
  const [data, { refetch }] = createResource(
    ...
    async ({ start, end }) => {
      try {
        ...
        return { txs: [] as Transaction[], exps: [] as Expense[] };
      }
    }
  );
  ...
  const handleExportTx = () => {
    if (txs().length === 0) { toast.warning("Tidak ada transaksi"); return; }
    exportTransactionsToExcel(txs(), "laporan-transaksi");
    toast.success("Transaksi diekspor");
  };

  const handleExportExp = () => {
    if (exps().length === 0) { toast.warning("Tidak ada pengeluaran"); return; }
    ...
    <div class={[styles.page, "page-enter"].join(" ")}>
      ...
      <div class={styles.periodCard}>
        <div class={styles.presets}>
          {(["daily", "monthly", "yearly", "custom"] as ReportType[]).map((t) => (
            <button
              class={[styles.presetBtn, reportType() === t && styles.presetActive].filter(Boolean).join(" ")}
              onClick={() => setPreset(t)}>
        ...
        <div class={styles.dateRange}>
          <Input label="Dari" type="date" value={startDate()}
            onInput={(e) => { setReportType("custom"); setStartDate(e.currentTarget.value); }} />
          <span class={styles.dateSep}>—</span>
          <Input label="Sampai" type="date" value={endDate()}
            onInput={(e) => { setReportType("custom"); setEndDate(e.currentTarget.value); }} />
          ...
          <div class={styles.exportCard}>
            ...
            <div class={styles.exportList}>
              {[
                ...
                <button class={styles.exportItem} onClick={item.fn}>
