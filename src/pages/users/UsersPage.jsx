...
const emptyForm = () => ({
    displayName: "", email: "", role: "kasir", isActive: true, password: "",
});
export default function UsersPage() {
    ...
    const [editingUid, setEditingUid] = createSignal(null);
    ...
    const handleSave = async () => {
        ...
        if (!editingUid() && !f.password) {
            toast.error("Password wajib diisi untuk akun baru");
            return;
        }
        if (!editingUid() && f.password.length < 6) {
    ...
    return (<div class={[styles.page, "page-enter"].join(" ")}>
      ...
      <Show when={!users.loading && (users() ?? []).length > 0}>
        <div class={styles.usersGrid}>
          <For each={users() ?? []}>
            {(u) => (<div class={[styles.userCard, !u.isActive && styles.userInactive].filter(Boolean).join(" ")}>
                ...
                ? (u.createdAt instanceof Date
                    ? u.createdAt
                    : u.createdAt.toDate?.() ?? new Date()).toLocaleDateString("id-ID", { day: "2-digit", month: "long", year: "numeric" })
                : "—"}
                  </p>
                </div>
      ...
      <Modal open={showModal()} onClose={() => setShowModal(false)} title={editingUid() ? "Edit Pengguna" : "Tambah Pengguna Baru"} size="sm" footer={<><Button variant="secondary" onClick={() => setShowModal(false)}>Batal</Button><Button loading={saving()} onClick={handleSave}>{editingUid() ? "Simpan" : "Buat Akun"}</Button></>}>
        <div class={styles.formGrid}>
          <Input label="Nama Lengkap" placeholder="cth. Siti Rahma" value={form().displayName} onInput={(e) => setForm((f) => ({ ...f, displayName: e.currentTarget.value }))} class={styles.colSpan2}/>
          <Input label="Email" type="email" placeholder="email@saulo.com" value={form().email} onInput={(e) => setForm((f) => ({ ...f, email: e.currentTarget.value }))} class={styles.colSpan2} disabled={!!editingUid()}/>
          <Show when={!editingUid()}>
            <Input label="Password" type="password" placeholder="Min. 6 karakter" value={form().password} onInput={(e) => setForm((f) => ({ ...f, password: e.currentTarget.value }))} class={styles.colSpan2}/>
          </Show>
          <Select label="Role" options={[{ value: "admin", label: "Admin" }, { value: "kasir", label: "Kasir" }]} value={form().role} onChange={(e) => setForm((f) => ({ ...f, role: e.currentTarget.value }))}/>
