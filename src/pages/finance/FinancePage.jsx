...
export default function FinancePage() {
    ...
    const margin = createMemo(() => totalIncome() > 0 ? ((netProfit() / totalIncome()) * 100).toFixed(1) : "0");
    ...
    return (<div class={[styles.page, "page-enter"].join(" ")}>
      ...
      <div class={styles.filterBar}>
        <Input label="Dari Tanggal" type="date" value={startDate()} onInput={(e) => setStartDate(e.currentTarget.value)} class={styles.dateInput}/>
        <Input label="Sampai Tanggal" type="date" value={endDate()} onInput={(e) => setEndDate(e.currentTarget.value)} class={styles.dateInput}/>
        <Button variant="secondary" size="md" class={styles.resetBtn} onClick={() => { setStartDate(getStartOfMonth()); setEndDate(getToday()); }}>
          Reset
        </Button>
      </div>
      ...
      <div class={styles.statsGrid}>
        <StatCard label="Total Pemasukan" value={formatCurrency(totalIncome())} color="success" icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>}/>
        <StatCard label="Total Pengeluaran" value={formatCurrency(totalExpense())} color="warning" icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6"/><polyline points="17 18 23 18 23 12"/></svg>}/>
        <StatCard label="Laba Bersih" value={formatCurrency(netProfit())} color={netProfit() >= 0 ? "primary" : "warning"} icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>}/>
        <StatCard label="Margin Laba" value={`${margin()}%`} color="gold" icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>}/>
      </div>
