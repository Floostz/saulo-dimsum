...
export default function DashboardPage() {
    ...
    const lowStock = createMemo(() => stocks().filter((s) => s.currentStock <= s.minStock));
    const todayProfit = createMemo(() => (stats()?.todayIncome ?? 0) - (stats()?.todayExpense ?? 0));
    const monthProfit = createMemo(() => (stats()?.monthIncome ?? 0) - (stats()?.monthExpense ?? 0));
    return (<div class={[styles.page, "page-enter"].join(" ")}>
      ...
      <div class={styles.statsGrid}>
        <Show when={stats.loading}>
          {[1, 2, 3, 4].map(() => (<div class={styles.skeletonCard}>
        ...
        <Show when={!stats.loading}>
          <StatCard label="Pemasukan Hari Ini" value={formatCurrency(stats()?.todayIncome ?? 0)} subValue={`${stats()?.transactionCount ?? 0} transaksi`} color="primary" icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>}/>
          <StatCard label="Pengeluaran Hari Ini" value={formatCurrency(stats()?.todayExpense ?? 0)} color="warning" icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>}/>
          <StatCard label="Laba Hari Ini" value={formatCurrency(todayProfit())} color={todayProfit() >= 0 ? "success" : "warning"} icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>}/>
          <StatCard label="Laba Bulan Ini" value={formatCurrency(monthProfit())} subValue={`Pendapatan ${formatCurrency(stats()?.monthIncome ?? 0)}`} color="gold" icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>}/>
        </Show>
      </div>
      ...
      <div class={styles.bottomRow}>
          ...
          <Show when={lowStock().length > 0}>
            <div class={styles.stockList}>
              <For each={lowStock().slice(0, 6)}>
                {(item) => (<div class={styles.stockRow}>
        ...
        <div class={styles.summaryCard}>
          ...
          <div class={styles.summaryList}>
            <Show when={stats.loading}>
              {[1, 2, 3, 4].map(() => (<div class={styles.summaryRow}>
        ...
        ].map((s) => (<div class={styles.summaryRow}>
        ...
        ].map((item) => (<a href={item.href} class={styles.quickNavItem}>
