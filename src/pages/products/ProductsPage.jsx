...
export default function ProductsPage() {
    ...
    const list = createMemo(() => (products() ?? []).filter((p) => {
    ...
    const margin = (p) => p.price > 0 ? ((p.price - p.costPrice) / p.price * 100).toFixed(0) + "%" : "—";
    return (<div class={[styles.page, "page-enter"].join(" ")}>
      ...
      <div class={styles.filters}>
        <Input placeholder="Cari produk..." value={search()} onInput={(e) => setSearch(e.currentTarget.value)} leftIcon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>} class={styles.searchBox}/>
      ...
      {/* Add/Edit Modal */}
      <Modal open={showModal()} onClose={() => setShowModal(false)} title={editingId() ? "Edit Produk" : "Tambah Produk Baru"} size="md" footer={<><Button variant="secondary" onClick={() => setShowModal(false)}>Batal</Button><Button loading={saving()} onClick={handleSave}>{editingId() ? "Simpan" : "Tambah"}</Button></>}>
        <div class={styles.formGrid}>
          <Input label="Nama Produk" placeholder="cth. Dimsum Ayam 6 pcs" value={form().name} onInput={(e) => setForm((f) => ({ ...f, name: e.currentTarget.value }))} class={styles.colSpan2}/>
          ...
          <Select label="Satuan" options={UNITS.map((u) => ({ value: u, label: u }))} value={form().unit} onChange={(e) => setForm((f) => ({ ...f, unit: e.currentTarget.value }))}/>
          <Input label="Harga Jual (Rp)" type="number" placeholder="0" value={form().price || ""} onInput={(e) => setForm((f) => ({ ...f, price: parseFloat(e.currentTarget.value) || 0 }))}/>
          <Input label="Harga Modal (Rp)" type="number" placeholder="0" value={form().costPrice || ""} onInput={(e) => setForm((f) => ({ ...f, costPrice: parseFloat(e.currentTarget.value) || 0 }))}/>
          <Input label="Stok Awal" type="number" placeholder="0" value={form().stock || ""} onInput={(e) => setForm((f) => ({ ...f, stock: parseInt(e.currentTarget.value) || 0 }))}/>
      ...
      <Modal open={!!showDelete()} onClose={() => setShowDelete(null)} title="Hapus Produk" size="sm" footer={<><Button variant="secondary" onClick={() => setShowDelete(null)}>Batal</Button><Button variant="danger" loading={deleting()} onClick={() => handleDelete(showDelete())}>Hapus</Button></>}>
